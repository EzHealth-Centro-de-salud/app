...
type Props = NativeStackScreenProps<RootStackParamList, "Confirm_Details">;

const Confirm_Details: React.FC<Props> = ({ navigation: { navigate } }) => {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [showAnimation, setShowAnimation] = useState(false);
  const [showFinishText, setShowFinishText] = useState(false);
  const [fadeAnim] = useState(new Animated.Value(0));
  const { appType, appPersonnel, appDate, appTime, userId } = useUserStore();
      ...
      <SafeAreaView>
        ...
        >
          ...
          >
            {!showAnimation && (
              <TouchableOpacity
                onPress={() => navigate("Create_Appointment_2")}
          ...
          {!showAnimation ? (
            <View>
              <Card style={{ margin: 10 }}>
                ...
                <Card.Content>
                  ...
                  <Text>
                    Médico: {appPersonnel.first_name}, {appPersonnel.surname}
                  </Text>
                </Card.Content>
              </Card>
              <TouchableOpacity
            ...
            >
              ...
              >
                {showFinishText ? "Cita agendada exitosamente" : ""}
              </Animated.Text>
            </View>
          )}
        </View>
